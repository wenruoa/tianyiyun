...
# 青龙面板配置：cron "30 4 * * *" script-path=xxx.py,tag=匹配cron用
# const $ = new Env('天翼云盘签到');
# 变量说明：
#   ty_username: 天翼云盘用户名(手机号)，多个账号用&隔开
#   ty_password: 天翼云盘密码，多个账号用&隔开
#   WXPUSHER_APP_TOKEN: WxPusher的APP_TOKEN
#   WXPUSHER_UID: WxPusher的用户UID，多个用&隔开
# 有图形验证码就是风控了 自己去网页端登陆 输入验证码 等几天
#设备锁问题请访问https://github.com/vistal8/tianyiyun/blob/main/README.md 查看详细说明，该地址为参考代码的作者提供
# 依赖安装：pip install requests rsa
# 依赖类型：第三方Python库
